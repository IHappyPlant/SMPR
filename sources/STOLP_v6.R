...
kwnn <- function(xl, z, k, q) {
  ...
  sum_by_class <- sapply(unique(names(weights)), function(class, weights) sum(weights[names(weights) == class]), weights)
...
stolp <- function(xl, err_lim = 5, noise_lim = -2) {
  l <- nrow(xl)
  n <- ncol(xl)
  omega <- data.frame()
  margins = rep(0, l)
  ...
  for (i in unique(new_xl[,n])) {
    ...
    omega <- rbind(omega, new_xl[indexes[which.max(margins)],])
    to_out <- c(to_out, indexes[which.max(margins)])
  }
